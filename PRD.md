...
Las instituciones financieras generan volúmenes altos de alertas AML por transacciones, que un analista de cumplimiento revisa: contexto del cliente, historial transaccional, reglas que dispararon la alerta, y decisión de cerrar, escalar o pedir más info. El cuello de botella no es la detección (la resuelven las reglas/motores existentes) — es la **investigación y triage manual**: lento, inconsistente entre analistas, y generador de backlog que en algunos bancos se mide en semanas. Eso implica riesgo regulatorio directo (SLAs de reporte a UIF) y costo operativo.
...
| **Riesgo regulatorio** | Es el punto donde el approval gate por código importa más: el agente nunca cierra una alerta ni dispara un reporte por sí solo, siempre recomienda con evidencia y el analista aprueba. |

### Alternativas descartadas

- **Aprobación de crédito**: mayor superficie de riesgo (dinero se mueve), requiere modelos de score financiero más complejos.
- **Verificación KYC**: es más un problema de extracción/verificación documental (OCR, matching de identidad).
- **Cobranza temprana**: menor riesgo regulatorio, pero también menor impacto/urgencia ejecutiva.
...
- **Secundario:** Oficial de cumplimiento — supervisa métricas de backlog y consistencia, consume reportes, no opera el agente directamente.
...
- El agente arma un caso: junta contexto del cliente, identifica la regla disparada, genera resumen de investigación + recomendación (cerrar / escalar a SAR / pedir info / bloquear cuenta) con evidencia. `bloquear_cuenta` es la recomendación reservada para casos de alta gravedad, donde además de reportar conviene congelar la cuenta preventivamente mientras avanza la investigación.
...
- Motor de detección de nuevas reglas.
- Multi-jurisdicción regulatoria.
- Aprendizaje continuo del agente en base a decisiones del analista.

---

## 4. Requisitos priorizados (MoSCoW)

- **Must:** aprobación humana obligatoria por código antes de cualquier efecto.
...
- Ninguna alerta se cierra ni se marca para reporte sin acción explícita del analista.
- El agente siempre devuelve evidencia trazable a datos concretos, no conclusiones sin sustento.
- Al menos un caso del set de evals donde el agente recomienda mal o pide escalar en falso.
...
3. **v3 (7-12 meses) — Reporte semi-asistido + dashboard.** El agente prepara el borrador de SAR para revisión humana; visibilidad agregada para el compliance officer.
...
- **Upsell:** módulo adicional dentro de contratos existentes, sin integración nueva de rieles de pago.
...
- **Calidad del modelo:** falsos negativos son el error más caro → mitigación: el agente nunca cierra, solo recomienda; los evals priorizan medir este error específico.
- **Adopción:** analistas aprobando "en automático" sin leer la evidencia → mitigación: mostrar siempre evidencia, no solo conclusión.
...
## 8. Registro de decisiones ante ambigüedad 

- **Tool use estructurado sin función ejecutable**, en vez de function-calling con flag de aprobación interno — el control queda garantizado por ausencia de superficie de acción.
- **Modo mock adicional al modo real** — permite reproducibilidad total del repo sin depender de una API key, a costa de no ser representativo de la calidad real de razonamiento.
- **SQLite en vez de estado en memoria** — agrega una dependencia mínima a cambio de auditoría persistente entre corridas.
